...
const PRODUCT_CATALOG_RETRY_ATTEMPTS = 3
const PRODUCT_CATALOG_RETRY_DELAY_MS = 500
...
function isRetryableAppsScriptError(error) {
  return error instanceof AppsScriptError && [502, 504].includes(error.statusCode)
}

async function callAppsScriptWithRetry(action, payload = {}, options = {}) {
  const {
    attempts = 1,
    initialDelayMs = 0,
  } = options
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      if (attempt >= attempts || !isRetryableAppsScriptError(error)) {
        throw error
      }
...
export async function getProducts() {
  const result = await callAppsScriptWithRetry('products', {}, {
    attempts: PRODUCT_CATALOG_RETRY_ATTEMPTS,
    initialDelayMs: PRODUCT_CATALOG_RETRY_DELAY_MS,
  })
...
export async function getBridgeHealth() {
  return callAppsScriptWithRetry('health', {}, {
    attempts: 2,
    initialDelayMs: 300,
  })
}
